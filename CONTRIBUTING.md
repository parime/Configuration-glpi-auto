...
Les tests d'intégration (`tests/Integration/`, contre une vraie instance) doivent tourner **depuis le conteneur GLPI**, car ils démarrent un vrai `Glpi\Kernel\Kernel` :
...
- **Analyse statique** : PHPStan est volontairement limité à `tests/Unit` (`phpstan.neon`), car tout le reste du code étend/instancie des classes du cœur GLPI qui n'existent que dans une instance réelle, non stubée pour l'analyse statique ici.
...
Integration tests (`tests/Integration/`, against a real instance) must run **from the GLPI container**, since they boot a real `Glpi\Kernel\Kernel`:
...
- **Static analysis**: PHPStan is deliberately limited to `tests/Unit` (`phpstan.neon`), as the rest of the code extends/instantiates GLPI core classes that only exist in a real instance, not stubbed for static analysis here.
