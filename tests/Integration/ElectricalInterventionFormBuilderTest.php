...
final class ElectricalInterventionFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new ElectricalInterventionFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToElectricityCategoryAndTitleIncludesLocation(): void
    {
        $form = $this->buildForm();
        ...
        $answers = $provider->getAnswers([
            "answers_$localisationId" => 'Salle de réunion Everest',
            "answers_$natureId" => ['2'],
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Électricité & Éclairage', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Salle de réunion Everest', $ticket->fields['name']);
    }
}
