...
final class RelocationArchivingFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new RelocationArchivingFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToRelocationCategoryAndTitleIncludesRequestType(): void
    {
        $form = $this->buildForm();
        ...
        $answers = $provider->getAnswers([
            "answers_$typeId" => ['2'],
            "answers_$localisationId" => 'Archives du sous-sol, bâtiment B',
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Déménagement & Archivage', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Archivage de documents', $ticket->fields['name']);
    }
}
