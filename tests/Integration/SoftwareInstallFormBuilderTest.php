...
final class SoftwareInstallFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new SoftwareInstallFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToInstallationCategoryAndTitleIncludesSoftwareName(): void
    {
        $form = $this->buildForm();
        ...
        $answers = $provider->getAnswers([
            "answers_$logicielId" => 'Adobe Acrobat Reader',
            "answers_$typeId" => ['1'],
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Installation / Désinstallation', 'itilcategories_id' => $logiciels->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Adobe Acrobat Reader', $ticket->fields['name']);
    }
}
