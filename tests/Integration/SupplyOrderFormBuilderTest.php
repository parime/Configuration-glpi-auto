...
final class SupplyOrderFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new SupplyOrderFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToSourcingCategoryAndTitleIncludesArticleAndQuantity(): void
    {
        $form = $this->buildForm();
        ...
        $answers = $provider->getAnswers([
            "answers_$articleId" => 'Casque téléphonique',
            "answers_$quantiteId" => '10',
        ]);
        ...
        $this->assertTrue($branch->getFromDBByCrit(['name' => 'Achats & Logistique', 'itilcategories_id' => 0]));
        $expectedCategory = new ITILCategory();
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Sourcing & Commande', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Casque téléphonique', $ticket->fields['name']);
        $this->assertStringContainsString('x10', $ticket->fields['name']);
    }
}
