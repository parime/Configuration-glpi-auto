...
final class WebsiteUpdateFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new WebsiteUpdateFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToWebsiteCategoryAndTitleIncludesPage(): void
    {
        $form = $this->buildForm();
        $pageId = $this->questionIdByRank($form, 0);
        $typeId = $this->questionIdByRank($form, 1);

        $provider = new EndUserInputNameProvider();
        $answers = $provider->getAnswers([
            "answers_$pageId" => 'Page "Contact"',
            "answers_$typeId" => ['2'],
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Site Web & Intranet', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Page "Contact"', $ticket->fields['name']);
    }
}
