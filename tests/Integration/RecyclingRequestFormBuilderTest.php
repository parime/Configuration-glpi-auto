...
final class RecyclingRequestFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new RecyclingRequestFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToRecyclingCategoryAndTitleIncludesWasteType(): void
    {
        $form = $this->buildForm();
        $typeId = $this->questionIdByRank($form, 0);
        $localisationId = $this->questionIdByRank($form, 1);

        $provider = new EndUserInputNameProvider();
        $answers = $provider->getAnswers([
            "answers_$typeId" => ['2'],
            "answers_$localisationId" => 'Local technique 2e étage',
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'RSE & Recyclage', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Déchets électroniques', $ticket->fields['name']);
    }
}
