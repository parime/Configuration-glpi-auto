 ...
 * reasoning. Submits the "Aménagement des horaires" branch (neither conditional field triggered) —
 * the observer mechanism is orthogonal to which motif was picked, so the simplest valid submission
 * is enough to exercise it.
 */
final class RemoteWorkFormBuilderTest extends TestCase
{
    private const FORM_NAME = "Demande de télétravail ou d'aménagement du temps de travail";

    private const AMENAGEMENT_OPTION_KEY = '3';
    ...
    private function buildForm(): Form
    {
        ...
        (new RemoteWorkFormBuilder())->build($config);

        $form = new Form();
        $this->assertTrue(
            $form->getFromDBByCrit(['name' => self::FORM_NAME]),
            'RemoteWorkFormBuilder should have created the form.'
        );
    ...
    {
        $form = $this->buildForm();
        $supervisorId = $this->addUser('phpunit_remote_supervisor');
        $requesterId = $this->addUser('phpunit_remote_requester', $supervisorId);

        $typeId = $this->questionIdByRank($form, 0);

        $ticket = $this->submitAsUser($requesterId, $form, [
            "answers_$typeId" => [self::AMENAGEMENT_OPTION_KEY],
        ]);
