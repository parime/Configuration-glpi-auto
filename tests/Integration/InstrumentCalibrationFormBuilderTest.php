...
/**
 * The "Référence ou numéro de série" question is optional — same "attach if you have it" style of
 * reasoning as the optional file questions elsewhere in this catalog, just for a text detail here.
 */
final class InstrumentCalibrationFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new InstrumentCalibrationFormBuilder())->build($config);
    ...
    public function testSubmissionWithoutTheOptionalReferenceStillValidatesAndRoutesToCalibrationCategory(): void
    {
        $form = $this->buildForm();
        $instrumentId = $this->questionIdByRank($form, 0);

        $provider = new EndUserInputNameProvider();
        $answers = $provider->getAnswers([
            "answers_$instrumentId" => 'Manomètre atelier 4',
        ]);

        $handler = AnswersHandler::getInstance();
        $validation = $handler->validateAnswers($form, $answers);
        $this->assertTrue($validation->isValid(), 'Submission without the optional reference should still validate: ' . json_encode($validation->getErrors()));
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Étalonnage & Métrologie', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Manomètre atelier 4', $ticket->fields['name']);
    }
}
