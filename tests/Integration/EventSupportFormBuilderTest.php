...
/**
 * "Type de support souhaité" is a `QuestionTypeCheckbox` (multi-select) — the real regression to
 * guard is that selecting several options at once is accepted and doesn't silently collapse to one.
 */
final class EventSupportFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new EventSupportFormBuilder())->build($config);
    ...
    public function testSubmissionWithMultipleSupportTypesRoutesToEventCategory(): void
    {
        $form = $this->buildForm();
        $nomId = $this->questionIdByRank($form, 0);
        $dateId = $this->questionIdByRank($form, 1);
        $typeId = $this->questionIdByRank($form, 2);

        $provider = new EndUserInputNameProvider();
        $answers = $provider->getAnswers([
            "answers_$nomId" => 'Journée portes ouvertes',
            "answers_$dateId" => '2026-11-15',
            "answers_$typeId" => ['1', '2'],
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Événementiel & Affichage', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Journée portes ouvertes', $ticket->fields['name']);
    }
}
