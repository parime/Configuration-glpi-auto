...
final class OfficeSuppliesFormBuilderTest extends TestCase
{
    ...
    private function buildConfig(): Config
    {
        ...
        $config->fields = array_merge(Config::getDefaults(), [
            'category_branches' => json_encode(['services_generaux']),
    ...
    private function buildForm(): Form
    {
        ...
        (new OfficeSuppliesFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToSuppliesCategoryAndTitleIncludesArticle(): void
    {
        $form = $this->buildForm();
        ...
        $answers = $provider->getAnswers([
            "answers_$articleId" => 'Ramette de papier A4',
            "answers_$quantiteId" => '5',
        ]);
        ...
        $this->assertTrue($branch->getFromDBByCrit(['name' => 'Services Généraux & Vie au Travail', 'itilcategories_id' => 0]));
        $expectedCategory = new ITILCategory();
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Consommables & Fournitures', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Ramette de papier A4', $ticket->fields['name']);
    }
}
