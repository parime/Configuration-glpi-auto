...
final class WaterLeakFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new WaterLeakFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToPlumbingCategoryAndTitleIncludesLocation(): void
    {
        $form = $this->buildForm();
        ...
        $answers = $provider->getAnswers([
            "answers_$localisationId" => 'Cuisine du 2e étage',
            "answers_$natureId" => ['1'],
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Plomberie & Sanitaires', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Cuisine du 2e étage', $ticket->fields['name']);
    }
}
