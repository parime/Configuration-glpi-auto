...
/**
 * The "Document à faire relire ou signer" question is optional — same "attach if you have it"
 * reasoning as `InvoicePaymentFormBuilderTest`.
 */
final class ContractReviewFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new ContractReviewFormBuilder())->build($config);
    ...
    public function testSubmissionWithoutTheOptionalDocumentStillValidatesAndRoutesToLegalCategory(): void
    {
        $form = $this->buildForm();
        $typeContratId = $this->questionIdByRank($form, 0);

        $provider = new EndUserInputNameProvider();
        $answers = $provider->getAnswers([
            "answers_$typeContratId" => 'Contrat de prestation de services',
        ]);

        $handler = AnswersHandler::getInstance();
        $validation = $handler->validateAnswers($form, $answers);
        $this->assertTrue($validation->isValid(), 'Submission without the optional document should still validate: ' . json_encode($validation->getErrors()));
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Juridique & Contrats', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Contrat de prestation de services', $ticket->fields['name']);
    }
}
