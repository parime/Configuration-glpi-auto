...
final class PreventiveMaintenanceFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new PreventiveMaintenanceFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToPreventiveMaintenanceCategoryAndTitleIncludesEquipment(): void
    {
        $form = $this->buildForm();
        ...
        $answers = $provider->getAnswers([
            "answers_$equipementId" => 'Convoyeur ligne 3',
            "answers_$periodeId" => '2026-12-10',
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Maintenance Préventive & Contrôles', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Convoyeur ligne 3', $ticket->fields['name']);
    }
}
