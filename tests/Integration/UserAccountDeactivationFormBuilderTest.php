...
final class UserAccountDeactivationFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new UserAccountDeactivationFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToOffboardingCategoryAndTitleIncludesName(): void
    {
        $form = $this->buildForm();
        $nomId = $this->questionIdByRank($form, 0);
        $dateFinId = $this->questionIdByRank($form, 1);
        $motifId = $this->questionIdByRank($form, 2);

        $provider = new EndUserInputNameProvider();
        $answers = $provider->getAnswers([
            "answers_$nomId" => 'Julien Roche',
            "answers_$dateFinId" => '2026-11-30',
            "answers_$motifId" => ['1'],
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Offboarding / Suppression', 'itilcategories_id' => $comptes->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Julien Roche', $ticket->fields['name']);
    }
}
