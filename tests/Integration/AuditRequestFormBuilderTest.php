...
final class AuditRequestFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new AuditRequestFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToAuditsCategoryAndTitleIncludesAuditType(): void
    {
        $form = $this->buildForm();
        ...
        $this->assertTrue($branch->getFromDBByCrit(['name' => 'Qualité, QHSE & Conformité', 'itilcategories_id' => 0]));
        $expectedCategory = new ITILCategory();
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Audits & Contrôles', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Contrôle réglementaire', $ticket->fields['name']);
    }
}
