...
final class SoftwareLicenseFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new SoftwareLicenseFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToLicensesCategoryAndTitleIncludesSoftwareAndQuantity(): void
    {
        $form = $this->buildForm();
        $logicielId = $this->questionIdByRank($form, 0);
        $quantiteId = $this->questionIdByRank($form, 1);

        $provider = new EndUserInputNameProvider();
        $answers = $provider->getAnswers([
            "answers_$logicielId" => 'Photoshop',
            "answers_$quantiteId" => '3',
        ]);
        ...
        $logiciels = new ITILCategory();
        $this->assertTrue($logiciels->getFromDBByCrit(['name' => 'Logiciels & Applications', 'itilcategories_id' => $branch->getID()]));
        $expectedCategory = new ITILCategory();
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Licences & Clés', 'itilcategories_id' => $logiciels->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Photoshop', $ticket->fields['name']);
        $this->assertStringContainsString('x3', $ticket->fields['name']);
    }
}
