...
final class WorkplaceSafetyFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new WorkplaceSafetyFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToHseCategoryAndTitleIncludesRequestType(): void
    {
        $form = $this->buildForm();
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Santé & Sécurité au Travail', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString("Déclaration d'un accident", $ticket->fields['name']);
    }
}
