...
final class BreakRoomFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new BreakRoomFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToBreakRoomCategoryAndTitleIncludesLocation(): void
    {
        $form = $this->buildForm();
        ...
        $answers = $provider->getAnswers([
            "answers_$localisationId" => 'Cafétéria RDC',
            "answers_$natureId" => ['1'],
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Pause & Restauration', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Cafétéria RDC', $ticket->fields['name']);
    }
}
