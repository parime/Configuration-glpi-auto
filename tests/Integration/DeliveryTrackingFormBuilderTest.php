...
final class DeliveryTrackingFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new DeliveryTrackingFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToDeliveryCategoryAndTitleIncludesReference(): void
    {
        $form = $this->buildForm();
        ...
        $answers = $provider->getAnswers([
            "answers_$referenceId" => 'BL-2026-9911',
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Réception, Livraison & Expédition', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('BL-2026-9911', $ticket->fields['name']);
    }
}
