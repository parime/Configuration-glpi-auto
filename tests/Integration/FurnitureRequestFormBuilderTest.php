...
final class FurnitureRequestFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new FurnitureRequestFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToFurnitureCategoryAndTitleIncludesFurnitureType(): void
    {
        $form = $this->buildForm();
        ...
        $answers = $provider->getAnswers([
            "answers_$typeId" => ['2'],
            "answers_$localisationId" => 'Bureau 214, 2e étage',
            "answers_$motifId" => ['3'],
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'Mobilier & Aménagement', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Siège', $ticket->fields['name']);
    }
}
