...
final class HeatingClimateFormBuilderTest extends TestCase
{
    ...
    private function buildForm(): Form
    {
        ...
        (new HeatingClimateFormBuilder())->build($config);
    ...
    public function testSubmissionRoutesToCvcCategoryAndTitleIncludesLocation(): void
    {
        $form = $this->buildForm();
        ...
        $answers = $provider->getAnswers([
            "answers_$localisationId" => 'Open space RH, 1er étage',
            "answers_$natureId" => ['3'],
        ]);
        ...
        $this->assertTrue($expectedCategory->getFromDBByCrit(['name' => 'CVC', 'itilcategories_id' => $branch->getID()]));

        $this->assertSame((int) $expectedCategory->getID(), (int) $ticket->fields['itilcategories_id']);
        $this->assertStringContainsString('Open space RH, 1er étage', $ticket->fields['name']);
    }
}
