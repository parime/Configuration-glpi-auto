 ...
 * maintainer request : "Demande de boîte mail ou d'alias" (IT / Messagerie & Collaboration /
 * Messagerie), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES`.
 *
 * No conditional question : unlike `MeetingRoomFormBuilder`'s two branches, all three request types
 * here (new mailbox, extra alias, shared team mailbox) share the exact same follow-up need — the
 * desired address or name — so a single radio question is enough, no branching required. A final
 ...
 */
class MailboxRequestFormBuilder
{
    private const FORM_NAME = "Demande de boîte mail ou d'alias";

    private const BRANCH_KEY = 'it';

    private const CATEGORY_PATH = ['Messagerie & Collaboration', 'Messagerie'];
    ...
    private const TYPE_OPTIONS = [
        '1' => 'Nouvelle boîte mail',
        '2' => 'Alias supplémentaire',
        '3' => 'Boîte mail partagée (équipe)',
    ];
    ...
    /**
     * @return array{type: Question, nomSouhaite: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $nomSouhaite = new Question();
        $nomSouhaite->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Adresse ou nom souhaité', 'configurationglpiauto'),
        ...
        if (!$type->getID() || !$nomSouhaite->getID() || !$precisions->getID()) {
            return null;
        }

        return ['type' => $type, 'nomSouhaite' => $nomSouhaite];
    }

    /**
     * @param array{type: Question, nomSouhaite: Question} $questions
     */
