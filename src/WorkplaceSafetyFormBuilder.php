 ...
 * request) : "Demande liée à la santé et sécurité au travail" (Sécurité & Protection des Personnes /
 * Santé & Sécurité au Travail), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES` (now flagged `'smart' => true` there and skipped by that
 * builder's own loop).
 *
 * No conditional branch : the four request types here (risk report, accident declaration, PPE
 * request, other) are handled by the same HSE team through the same intake process, and each is
 * already self-explanatory from its label — the details that would otherwise justify a follow-up
 * field belong in the free-text "Précisions complémentaires" field every class in this generalization
 * batch adds last, replacing the generic Description field these smart forms no longer have.
 */
class WorkplaceSafetyFormBuilder
{
    private const FORM_NAME = 'Demande liée à la santé et sécurité au travail';

    private const BRANCH_KEY = 'securite';

    private const CATEGORY_PATH = ['Santé & Sécurité au Travail'];
    ...
    private const TYPE_OPTIONS = [
        '1' => "Signalement d'un risque ou danger",
        '2' => "Déclaration d'un accident du travail",
        '3' => 'Demande liée aux équipements de protection (EPI)',
        '4' => 'Autre',
    ];
    ...
    /**
     * @return array{type: Question, precisions: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        if (!$type->getID() || !$precisions->getID()) {
            return null;
        }

        return ['type' => $type, 'precisions' => $precisions];
    }

    /**
     * @param array{type: Question, precisions: Question} $questions
     */
