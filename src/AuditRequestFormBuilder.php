 ...
 * maintainer request : "Demande liée à un audit ou un contrôle" (Qualité, QHSE & Conformité / Audits
 * & Contrôles), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES`.
 *
 * No conditional question : which kind of audit is expected and the period it's expected in are
 * both always relevant together, same "all fields answered every time" shape as
 ...
 * longer have.
 */
class AuditRequestFormBuilder
{
    private const FORM_NAME = 'Demande liée à un audit ou un contrôle';

    private const BRANCH_KEY = 'qualite';

    private const CATEGORY_PATH = ['Audits & Contrôles'];

    // Same icon `ServiceCatalogBuilder::BRANCH_ILLUSTRATIONS['qualite']` already gave this branch's
    // other forms.
    private const ILLUSTRATION = 'diagnostic';
    ...
    /**
     * @return array{type: Question, periode: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $type->add([
            ...
            'name' => __("Type d'audit", 'configurationglpiauto'),
        ...
        $periode = new Question();
        $periode->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Période souhaitée', 'configurationglpiauto'),
        ...
        if (!$type->getID() || !$periode->getID() || !$precisions->getID()) {
            return null;
        }

        return ['type' => $type, 'periode' => $periode];
    }

    /**
     * @param array{type: Question, periode: Question} $questions
     */
