 ...
 * request) : "Demande de formation" (RH / Formation & Montée en compétences), replacing the plain
 * Title+Description entry that used to live in `ServiceCatalogBuilder::SERVICES` (now flagged
 * `'smart' => true` there and skipped by that builder's own loop).
 *
 * No conditional branch : whatever the training, the requester always needs to say what it is and
...
class TrainingRequestFormBuilder
{
    ...
    private const CATEGORY_PATH = ['Formation & Montée en compétences'];
    ...
    /**
     * @return array{intitule: Question, periode: Question, precisions: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $intitule->add([
            ...
            'name' => __('Intitulé de la formation souhaitée', 'configurationglpiauto'),
        ...
        if (!$intitule->getID() || !$periode->getID() || !$precisions->getID()) {
            return null;
        }

        return ['intitule' => $intitule, 'periode' => $periode, 'precisions' => $precisions];
    }

    /**
     * @param array{intitule: Question, periode: Question, precisions: Question} $questions
     */
    private function configureDestination(Form $form, int $itilCategoryId, array $questions): void
    {
        ...
        // "<Nom du formulaire> - <intitulé>"
        $titleValue = $formProvider->getTagForForm($form)->html
            . ' - ' . $answerProvider->getTagForQuestion($questions['intitule'])->html;
