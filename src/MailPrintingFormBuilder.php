 ...
 * the plugin owner's explicit request to extend the six-service pilot everywhere) : "Envoi de
 * courrier ou demande de reprographie" (Administratif, Juridique & Finance / Courrier &
 * Reprographie), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES` (now flagged `'smart' => true` there and skipped by that
 * builder's own loop).
 *
 * This service bundles two genuinely different needs under one name, same trigger
 * `MeetingRoomFormBuilder` and `VpnAccessFormBuilder` already used for their own conditional branch :
 * a recipient and mailing address only make sense for the "envoi de courrier" branch, reprographie
 * requests need no such field. One `Question::visibility_strategy = VISIBLE_IF` branch, same
 * `Glpi\Form\Condition\ConditionData` mechanism as `VpnAccessFormBuilder` (see that class's docblock
 * for the full radio-option-value reasoning).
 *
 * Computed title deliberately only references the always-visible "type" answer, not the conditional
 * "destinataire" field : a reprographie request never answers it, so folding it into the title would
 * render an empty tag half the time. Built the same way as the pilot
 * (`AbroadMissionFormBuilder`, issue #208) : `AnswerTagProvider::getTagForQuestion()` /
 ...
 */
class MailPrintingFormBuilder
{
    private const FORM_NAME = 'Envoi de courrier ou demande de reprographie';

    private const BRANCH_KEY = 'administratif';

    private const CATEGORY_PATH = ['Courrier & Reprographie'];

    // Same icon `ServiceCatalogBuilder::BRANCH_ILLUSTRATIONS['administratif']` already gave this
    // branch's other forms.
    private const ILLUSTRATION = 'legal';

    private const TYPE_OPTIONS = [
        '1' => 'Envoi de courrier',
        '2' => 'Reprographie (impressions, photocopies)',
    ];

    private const MAIL_OPTION_KEY = '1';
    ...
    private function addQuestions(Form $form): ?array
    {
        ...
        $mailCondition = new ConditionData(
            item_uuid: $typeUuid,
            item_type: ConditionType::QUESTION->value,
            value_operator: ValueOperator::EQUALS->value,
            value: self::MAIL_OPTION_KEY,
        );

        $destinataire = new Question();
        $destinataire->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Destinataire et adresse', 'configurationglpiauto'),
            'type' => QuestionTypeShortText::class,
            'is_mandatory' => 1,
            'vertical_rank' => 1,
            'visibility_strategy' => VisibilityStrategy::VISIBLE_IF->value,
            'conditions' => json_encode([$mailCondition->jsonSerialize()]),
        ]);
        ...
        if (!$destinataire->getID() || !$precisions->getID()) {
            return null;
        }
