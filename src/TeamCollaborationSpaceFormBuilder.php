 ...
 * maintainer request : "Demande d'accès à un espace collaboratif d'équipe" (IT / Messagerie &
 * Collaboration / Collaboration), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES`.
 *
 * No conditional question : the team/project name and the desired access level are both always
 * relevant, same "all fields answered every time" shape as `AccessBadgeFormBuilder`. A final
 * free-text "Précisions complémentaires" field is added on every class in this generalization pass,
 * replacing the generic Description field these smart forms no longer have.
 */
class TeamCollaborationSpaceFormBuilder
{
    private const FORM_NAME = "Demande d'accès à un espace collaboratif d'équipe";

    private const BRANCH_KEY = 'it';

    private const CATEGORY_PATH = ['Messagerie & Collaboration', 'Collaboration'];
    ...
    private const TYPE_ACCES_OPTIONS = [
        '1' => 'Lecture seule',
    ...
    /**
     * @return array{nomEquipe: Question, typeAcces: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $nomEquipe = new Question();
        $nomEquipe->add([
            'forms_sections_id' => $sectionId,
            'name' => __("Nom de l'équipe ou du projet", 'configurationglpiauto'),
            'type' => QuestionTypeShortText::class,
            'is_mandatory' => 1,
            'vertical_rank' => 0,
        ]);

        $typeAcces = new Question();
        $typeAcces->add([
            'forms_sections_id' => $sectionId,
            'name' => __("Type d'accès souhaité", 'configurationglpiauto'),
            'type' => QuestionTypeRadio::class,
            'is_mandatory' => 1,
            'vertical_rank' => 1,
            'extra_data' => json_encode((new QuestionTypeSelectableExtraDataConfig(
                options: self::TYPE_ACCES_OPTIONS,
            ))->jsonSerialize()),
        ]);
        ...
        if (!$nomEquipe->getID() || !$typeAcces->getID() || !$precisions->getID()) {
            return null;
        }

        return ['nomEquipe' => $nomEquipe, 'typeAcces' => $typeAcces];
    }

    /**
     * @param array{nomEquipe: Question, typeAcces: Question} $questions
     */
    private function configureDestination(Form $form, int $itilCategoryId, array $questions): void
    {
        ...
        // "<Nom du formulaire> - <nomEquipe>"
        $titleValue = $formProvider->getTagForForm($form)->html
            . ' - ' . $answerProvider->getTagForQuestion($questions['nomEquipe'])->html;
