 ...
 * maintainer request : "Installation ou mise à jour d'un logiciel" (IT / Logiciels & Applications /
 * Installation / Désinstallation), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES`.
 *
 * No conditional question : knowing which software is affected and whether it's a fresh install or
 * an update are both always relevant, same "all fields answered every time" shape as
 ...
 * longer have.
 */
class SoftwareInstallFormBuilder
{
    private const FORM_NAME = "Installation ou mise à jour d'un logiciel";

    private const BRANCH_KEY = 'it';

    private const CATEGORY_PATH = ['Logiciels & Applications', 'Installation / Désinstallation'];
    ...
    private const TYPE_OPTIONS = [
        '1' => "Installation d'un nouveau logiciel",
        '2' => "Mise à jour d'un logiciel existant",
    ];
    ...
    /**
     * @return array{logiciel: Question, type: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        if (!$logiciel->getID() || !$type->getID() || !$precisions->getID()) {
            return null;
        }

        return ['logiciel' => $logiciel, 'type' => $type];
    }

    /**
     * @param array{logiciel: Question, type: Question} $questions
     */
