 ...
 * the plugin owner's explicit request to extend the six-service pilot everywhere) : "Demande liée à
 * une facture ou un paiement" (Administratif, Juridique & Finance / Finance & Comptabilité),
 ...
 *
 * A finance request always names a reference and a request type, no branch needed there, but the
 * supporting document (the invoice or purchase order itself) genuinely isn't always at hand when the
 * ticket is opened, so it stays an optional `QuestionTypeFile` rather than a mandatory one — same
 * "attach if you have it" reasoning as any other optional upload in this catalog. Computed title
 * built the same way as the pilot (`AbroadMissionFormBuilder`, issue #208) :
...
class InvoicePaymentFormBuilder
{
    ...
    private const TYPE_OPTIONS = [
        '1' => 'Question sur une facture reçue',
        '2' => 'Demande de paiement fournisseur',
        '3' => 'Autre',
    ];
    ...
    /**
     * @return array{reference: Question, type: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $reference = new Question();
        $reference->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Numéro de facture ou référence', 'configurationglpiauto'),
        ...
        $justificatif->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Justificatif (facture, bon de commande...)', 'configurationglpiauto'),
        ...
        if (!$reference->getID() || !$type->getID() || !$justificatif->getID() || !$precisions->getID()) {
            return null;
        }

        return ['reference' => $reference, 'type' => $type];
    }

    /**
     * @param array{reference: Question, type: Question} $questions
     */
    private function configureDestination(Form $form, int $itilCategoryId, array $questions): void
    {
        ...
        // "<Nom du formulaire> - <référence>"
        $titleValue = $formProvider->getTagForForm($form)->html
            . ' - ' . $answerProvider->getTagForQuestion($questions['reference'])->html;
