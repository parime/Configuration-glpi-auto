...
/**
 * One of the ~6 service-catalog upgrades for issue #207 : "Demande de licence logicielle" (IT /
 * Logiciels & Applications / Licences & Clés), replacing the plain Title+Description entry that
 * used to live in `ServiceCatalogBuilder::SERVICES` (now flagged `'smart' => true` there and
 * skipped by that builder's own loop).
 *
 * Computed title only, no conditional question needed : the product name and quantity are the two
 * facts that make this kind of ticket meaningfully identifiable in a list at a glance ("Photoshop
 * x3" vs. a generic "Demande de licence logicielle" repeated 40 times), same reasoning the issue
 * itself gives for the pilot's dates. `QuestionTypeNumber` needs no `extra_data` at all (confirmed
 * reading GLPI 11 core : it extends `AbstractQuestionTypeShortAnswer` directly, unlike the date/
 * selectable types this plugin's other new builders configure), so this is the simplest of the six.
 * Title built the exact same way as the pilot (`AbroadMissionFormBuilder`,
 * `AnswerTagProvider::getTagForQuestion()` / `FormTagProvider::getTagForForm()`, never hand-written
 * `<span>` markup).
 */
class SoftwareLicenseFormBuilder
{
    private const FORM_NAME = 'Demande de licence logicielle';

    private const BRANCH_KEY = 'it';

    private const CATEGORY_PATH = ['Logiciels & Applications', 'Licences & Clés'];
    ...
    /**
     * @return array{logiciel: Question, quantite: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $quantite = new Question();
        $quantite->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Quantité de licences', 'configurationglpiauto'),
        ...
        $precisions->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Précisions', 'configurationglpiauto'),
            'type' => QuestionTypeLongText::class,
            'is_mandatory' => 0,
            'vertical_rank' => 2,
        ]);

        if (!$logiciel->getID() || !$quantite->getID() || !$precisions->getID()) {
            return null;
        }

        return ['logiciel' => $logiciel, 'quantite' => $quantite];
    }

    /**
     * @param array{logiciel: Question, quantite: Question} $questions
     */
