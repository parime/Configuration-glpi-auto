 ...
 * modification sur le site web ou intranet" (Communication & Marketing / Site Web & Intranet),
 ...
 *
 * Which page and what kind of change are both always relevant, no branch needed : the radio narrows
 * the request to a known bucket (bug fix vs. content update) while the page field stays meaningful
 * regardless of which is picked. Same "all fields answered every time" shape as the pilot
 ...
 */
class WebsiteUpdateFormBuilder
{
    private const FORM_NAME = 'Demande de modification sur le site web ou intranet';

    private const BRANCH_KEY = 'communication';

    private const CATEGORY_PATH = ['Site Web & Intranet'];
    ...
    private const TYPE_OPTIONS = [
        ...
        '2' => 'Ajout ou mise à jour de contenu',
        '3' => 'Autre',
    ];
    ...
    /**
     * @return array{page: Question, type: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $page->add([
            ...
            'name' => __('Page ou section concernée', 'configurationglpiauto'),
        ...
        $type->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Type de modification', 'configurationglpiauto'),
        ...
        if (!$page->getID() || !$type->getID() || !$precisions->getID()) {
            return null;
        }
    ...
    /**
     * @param array{page: Question, type: Question} $questions
     */
    private function configureDestination(Form $form, int $itilCategoryId, array $questions): void
    {
        ...
        // "<Nom du formulaire> - <page>"
        $titleValue = $formProvider->getTagForForm($form)->html
            . ' - ' . $answerProvider->getTagForQuestion($questions['page'])->html;
