 ...
 * the plugin owner's explicit request to extend the six-service pilot everywhere) : "Signaler un
 * problème lié à la pause ou à la restauration" (Services Généraux & Vie au Travail / Pause &
 * Restauration), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES` (now flagged `'smart' => true` there and skipped by that
 * builder's own loop).
 *
 * Where and what's wrong are both always relevant, no branch needed : the radio narrows the report
 * to a known bucket (machine down / cleanliness / other) while staying just as informative whichever
 * option is picked, so no field needs hiding. Same "all fields answered every time" shape as the
 * pilot (`AbroadMissionFormBuilder`, issue #208), computed title built the same way
 ...
 */
class BreakRoomFormBuilder
{
    private const FORM_NAME = 'Signaler un problème lié à la pause ou à la restauration';

    private const BRANCH_KEY = 'services_generaux';

    private const CATEGORY_PATH = ['Pause & Restauration'];
    ...
    private const NATURE_OPTIONS = [
        '1' => 'Distributeur ou machine en panne',
        '2' => "Propreté de l'espace",
        '3' => 'Autre',
    ];
    ...
    private function addQuestions(Form $form): ?array
    {
        ...
        $localisation->add([
            ...
            'name' => __('Espace concerné', 'configurationglpiauto'),
