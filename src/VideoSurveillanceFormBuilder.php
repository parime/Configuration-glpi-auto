 ...
 * request) : "Signaler un dysfonctionnement vidéosurveillance ou alarme" (Sécurité & Protection des
 * Personnes / Vidéosurveillance & Alarmes), replacing the plain Title+Description entry that used to
 * live in `ServiceCatalogBuilder::SERVICES` (now flagged `'smart' => true` there and skipped by that
 * builder's own loop).
 *
 * No conditional branch : whatever the fault (camera down, false alarm, other), the security team
 * always needs to know where the equipment is and what's wrong with it, so both fields stay relevant
...
class VideoSurveillanceFormBuilder
{
    ...
    private const TYPE_OPTIONS = [
        '1' => 'Caméra hors service',
        '2' => 'Alarme qui se déclenche à tort',
    ...
    private function addQuestions(Form $form): ?array
    {
        ...
        $localisation->add([
            ...
            'name' => __("Localisation de l'équipement concerné", 'configurationglpiauto'),
        ...
        $type->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Type de dysfonctionnement', 'configurationglpiauto'),
        ...
        return ['localisation' => $localisation, 'type' => $type, 'precisions' => $precisions];
    }

    /**
     * @param array{localisation: Question, type: Question, precisions: Question} $questions
     */
