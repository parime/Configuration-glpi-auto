 ...
 * relecture ou signature de contrat" (Administratif, Juridique & Finance / Juridique & Contrats),
 ...
 *
 * The contract type is always relevant, no branch needed. The document itself stays an optional
 * `QuestionTypeFile` rather than mandatory : requesters sometimes open the ticket to *ask* for a
 * contract to be drafted or need a physical copy routed separately, same "attach if you have it"
 * reasoning as `InvoicePaymentFormBuilder`'s justificatif field. Computed title built the same way as
 * the pilot (`AbroadMissionFormBuilder`, issue #208) : `AnswerTagProvider::getTagForQuestion()` /
 ...
 */
class ContractReviewFormBuilder
{
    private const FORM_NAME = 'Demande de relecture ou signature de contrat';

    private const BRANCH_KEY = 'administratif';

    private const CATEGORY_PATH = ['Juridique & Contrats'];
    ...
    /**
     * @return array{typeContrat: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $typeContrat = new Question();
        $typeContrat->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Type de contrat concerné', 'configurationglpiauto'),
        ...
        $document->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Document à faire relire ou signer', 'configurationglpiauto'),
        ...
        if (!$typeContrat->getID() || !$document->getID() || !$precisions->getID()) {
            return null;
        }
    ...
    /**
     * @param array{typeContrat: Question} $questions
     */
    private function configureDestination(Form $form, int $itilCategoryId, array $questions): void
    {
        ...
        // "<Nom du formulaire> - <typeContrat>"
        $titleValue = $formProvider->getTagForForm($form)->html
            . ' - ' . $answerProvider->getTagForQuestion($questions['typeContrat'])->html;
