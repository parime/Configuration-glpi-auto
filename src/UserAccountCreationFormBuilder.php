 ...
 * maintainer request : "Création d'un compte utilisateur" (IT / Comptes & Identités / Onboarding /
 * Création de compte), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES`.
 *
 * No conditional question : an onboarding request always needs the same three facts (who, when
 * they start, what role) regardless of context, same "all fields answered every time" shape as
 ...
 * longer have.
 */
class UserAccountCreationFormBuilder
{
    private const FORM_NAME = "Création d'un compte utilisateur";

    private const BRANCH_KEY = 'it';

    private const CATEGORY_PATH = ['Comptes & Identités', 'Onboarding / Création de compte'];
    ...
    /**
     * @return array{nom: Question, arrivee: Question, poste: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $nom->add([
            ...
            'name' => __('Nom complet de la personne concernée', 'configurationglpiauto'),
            'type' => QuestionTypeShortText::class,
            'is_mandatory' => 1,
            'vertical_rank' => 0,
        ]);

        $arrivee = new Question();
        $arrivee->add([
            'forms_sections_id' => $sectionId,
            'name' => __("Date d'arrivée souhaitée", 'configurationglpiauto'),
        ...
        $poste->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Poste ou service', 'configurationglpiauto'),
            'type' => QuestionTypeShortText::class,
            'is_mandatory' => 1,
            'vertical_rank' => 2,
        ]);
        ...
        if (!$nom->getID() || !$arrivee->getID() || !$poste->getID() || !$precisions->getID()) {
            return null;
        }

        return ['nom' => $nom, 'arrivee' => $arrivee, 'poste' => $poste];
    }

    /**
     * @param array{nom: Question, arrivee: Question, poste: Question} $questions
     */
