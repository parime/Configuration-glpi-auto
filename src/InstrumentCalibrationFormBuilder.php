 ...
 * maintainer request : "Demande d'étalonnage d'un instrument" (Maintenance Industrielle & Technique /
 * Étalonnage & Métrologie), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES`.
 *
 * No conditional question : the instrument concerned is always the core of the request, and its
 * reference or serial number is a simple optional detail that helps identify it, not a branch that
 * changes what else is asked, same "all fields answered every time" shape as
 ...
 * longer have.
 */
class InstrumentCalibrationFormBuilder
{
    private const FORM_NAME = "Demande d'étalonnage d'un instrument";

    private const BRANCH_KEY = 'maintenance';

    private const CATEGORY_PATH = ['Étalonnage & Métrologie'];
    ...
    /**
     * @return array{instrument: Question, reference: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $instrument->add([
            ...
            'name' => __('Instrument concerné', 'configurationglpiauto'),
        ...
        $reference->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Référence ou numéro de série', 'configurationglpiauto'),
            'type' => QuestionTypeShortText::class,
            'is_mandatory' => 0,
            'vertical_rank' => 1,
        ]);
        ...
        if (!$instrument->getID() || !$reference->getID() || !$precisions->getID()) {
            return null;
        }

        return ['instrument' => $instrument, 'reference' => $reference];
    }

    /**
     * @param array{instrument: Question, reference: Question} $questions
     */
    private function configureDestination(Form $form, int $itilCategoryId, array $questions): void
    {
        ...
        // "<Nom du formulaire> - <instrument>"
        $titleValue = $formProvider->getTagForForm($form)->html
            . ' - ' . $answerProvider->getTagForQuestion($questions['instrument'])->html;
