 ...
 * catalog per explicit maintainer request : "Demande d'intervention électricité" (Bâtiment & Moyens
 * Généraux / Électricité & Éclairage), replacing the plain Title+Description entry that used to
 * live in `ServiceCatalogBuilder::SERVICES` (now flagged `'smart' => true` there and skipped by
 * that builder's own loop).
 *
 * Same shape as `HeatingClimateFormBuilder` : where and what, no conditional branch since the four
 * electrical outcomes (power cut, faulty outlet/switch, faulty lighting, other) don't need
 * different follow-up questions from each other. Every smart form in this generalization also gets
 * a final free-text "Précisions complémentaires" question as an escape hatch, replacing the generic
 ...
 */
class ElectricalInterventionFormBuilder
{
    private const FORM_NAME = "Demande d'intervention électricité";

    private const BRANCH_KEY = 'batiment';

    private const CATEGORY_PATH = ['Électricité & Éclairage'];
    ...
    private const NATURE_OPTIONS = [
        ...
        '2' => 'Prise ou interrupteur défectueux',
        '3' => 'Éclairage défectueux',
        '4' => 'Autre',
    ];
