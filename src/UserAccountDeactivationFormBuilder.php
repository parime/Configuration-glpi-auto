 ...
 * maintainer request : "Désactivation d'un compte utilisateur" (IT / Comptes & Identités /
 * Offboarding / Suppression), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES`.
 *
 * No conditional question : an offboarding request always needs who is leaving, when access should
 * end, and why, regardless of context, same "all fields answered every time" shape as
 * `UserAccountCreationFormBuilder`, its onboarding counterpart. A final free-text "Précisions
 ...
 */
class UserAccountDeactivationFormBuilder
{
    private const FORM_NAME = "Désactivation d'un compte utilisateur";

    private const BRANCH_KEY = 'it';

    private const CATEGORY_PATH = ['Comptes & Identités', 'Offboarding / Suppression'];
    ...
    private const MOTIF_OPTIONS = [
        '1' => 'Départ définitif',
        '2' => 'Fin de mission ou de contrat',
        '3' => 'Autre',
    ];
    ...
    /**
     * @return array{nom: Question, dateFin: Question, motif: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $nom->add([
            ...
            'name' => __('Nom de la personne concernée', 'configurationglpiauto'),
        ...
        if (!$nom->getID() || !$dateFin->getID() || !$motif->getID() || !$precisions->getID()) {
            return null;
        }

        return ['nom' => $nom, 'dateFin' => $dateFin, 'motif' => $motif];
    }

    /**
     * @param array{nom: Question, dateFin: Question, motif: Question} $questions
     */
