 ...
 * maintainer request : "Demande de maintenance préventive" (Maintenance Industrielle & Technique /
 * Maintenance Préventive & Contrôles), replacing the plain Title+Description entry that used to live
 * in `ServiceCatalogBuilder::SERVICES`.
 *
 * No conditional question : which equipment is concerned and when the intervention is wanted are
 ...
 * longer have.
 */
class PreventiveMaintenanceFormBuilder
{
    private const FORM_NAME = 'Demande de maintenance préventive';

    private const BRANCH_KEY = 'maintenance';

    private const CATEGORY_PATH = ['Maintenance Préventive & Contrôles'];
    ...
    /**
     * @return array{equipement: Question, periode: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        if (!$equipement->getID() || !$periode->getID() || !$precisions->getID()) {
            return null;
        }

        return ['equipement' => $equipement, 'periode' => $periode];
    }

    /**
     * @param array{equipement: Question, periode: Question} $questions
     */
