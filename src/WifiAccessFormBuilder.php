 ...
 * maintainer request : "Demande d'accès Wifi" (IT / Réseau & Connectivité / Wifi), replacing the
 * plain Title+Description entry that used to live in `ServiceCatalogBuilder::SERVICES`.
 *
 * Same shape as `VpnAccessFormBuilder` : whether the access is permanent (a collaborator) or
 * temporary (a guest) decides if an end date makes sense at all, so that single field is gated
 * behind `Question::visibility_strategy = VISIBLE_IF` + `Glpi\Form\Condition\ConditionData` on the
 * "temporary" option, same mechanism, condition `value` is the option's key not its label.
 *
 * Computed title only references the always-visible "type" answer, not the conditional end date :
 * a permanent request never answers that field, so folding it into the title would render an empty
 * tag half the time.
 */
class WifiAccessFormBuilder
{
    private const FORM_NAME = "Demande d'accès Wifi";

    private const BRANCH_KEY = 'it';

    private const CATEGORY_PATH = ['Réseau & Connectivité', 'Wifi'];
    ...
    private const TYPE_OPTIONS = [
        '1' => 'Accès permanent (collaborateur)',
        '2' => 'Accès temporaire (invité)',
    ];

    private const TEMPORARY_OPTION_KEY = '2';
    ...
    /**
     * @return array{type: Question, appareil: Question, dateFin: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $type->add([
            ...
            'name' => __("Type d'accès", 'configurationglpiauto'),
        ...
        $appareil = new Question();
        $appareil->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Appareil ou nom du visiteur', 'configurationglpiauto'),
            'type' => QuestionTypeShortText::class,
            'is_mandatory' => 1,
            'vertical_rank' => 1,
        ]);

        $dateFin = new Question();
        $dateFin->add([
            'forms_sections_id' => $sectionId,
        ...
        if (!$appareil->getID() || !$dateFin->getID() || !$precisions->getID()) {
            return null;
        }

        return ['type' => $type, 'appareil' => $appareil, 'dateFin' => $dateFin];
    }

    /**
     * @param array{type: Question, appareil: Question, dateFin: Question} $questions
     */
