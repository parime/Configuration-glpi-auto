 ...
 * request) : "Suivi de réception ou de livraison" (Achats & Logistique / Réception, Livraison &
 * Expédition), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES` (now flagged `'smart' => true` there and skipped by that
 * builder's own loop).
 *
 * A single structured field, no conditional branch : a reference number is genuinely the whole point
 * of this service (it's what the recipient looks up to answer the request), so one mandatory
 * ShortText plus the free-text escape hatch is enough — there's no natural second axis to branch on.
 * Also carries the "Précisions complémentaires" free-text field every class in this generalization
 * batch adds last, replacing the generic Description field these smart forms no longer have.
 */
class DeliveryTrackingFormBuilder
{
    private const FORM_NAME = 'Suivi de réception ou de livraison';

    private const BRANCH_KEY = 'achats';

    private const CATEGORY_PATH = ['Réception, Livraison & Expédition'];
    ...
    /**
     * @return array{reference: Question, precisions: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $reference->add([
            ...
            'name' => __('Numéro de commande ou de bon de livraison', 'configurationglpiauto'),
        ...
        if (!$reference->getID() || !$precisions->getID()) {
            return null;
        }

        return ['reference' => $reference, 'precisions' => $precisions];
    }

    /**
     * @param array{reference: Question, precisions: Question} $questions
     */
