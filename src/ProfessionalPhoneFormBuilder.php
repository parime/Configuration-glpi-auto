 ...
 * maintainer request : "Demande de téléphone professionnel" (IT / Téléphonie & VoIP / Smartphone &
 * Flotte mobile), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES`.
 *
 * Same shape as `LaptopRequestFormBuilder` : the device type is always relevant, but only a
 * "replacement" motif needs a follow-up naming the device being replaced. That single field is gated
 * behind `Question::visibility_strategy = VISIBLE_IF` + `Glpi\Form\Condition\ConditionData` on the
 * "replacement" option, same mechanism, condition `value` is the option's key not its label.
 *
...
class ProfessionalPhoneFormBuilder
{
    ...
    private const CATEGORY_PATH = ['Téléphonie & VoIP', 'Smartphone & Flotte mobile'];
    ...
    private const TYPE_APPAREIL_OPTIONS = [
        '1' => 'Smartphone',
        '2' => 'Téléphone basique',
    ];
    ...
    private const MOTIF_OPTIONS = [
        ...
        '2' => "Remplacement d'un appareil existant",
    ];
    ...
    /**
     * @return array{typeApp: Question, motif: Question, modeleActuel: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        if (!$typeApp->getID() || !$motif->getID()) {
            return null;
        }
        $motif->getFromDB($motif->getID());
        $motifUuid = $motif->fields['uuid'];

        $remplacementCondition = new ConditionData(
            item_uuid: $motifUuid,
        ...
        if (!$modeleActuel->getID() || !$precisions->getID()) {
            return null;
        }

        return ['typeApp' => $typeApp, 'motif' => $motif, 'modeleActuel' => $modeleActuel];
    }

    /**
     * @param array{typeApp: Question, motif: Question, modeleActuel: Question} $questions
     */
    private function configureDestination(Form $form, int $itilCategoryId, array $questions): void
    {
        ...
        // "<Nom du formulaire> - <typeApp>"
        $titleValue = $formProvider->getTagForForm($form)->html
            . ' - ' . $answerProvider->getTagForQuestion($questions['typeApp'])->html;
