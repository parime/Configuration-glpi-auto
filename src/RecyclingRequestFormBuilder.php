 ...
 * the plugin owner's explicit request to extend the six-service pilot everywhere) : "Demande liée
 * au tri ou au recyclage" (Services Généraux & Vie au Travail / RSE & Recyclage), replacing the
 ...
 *
 * What kind of waste and where to collect it are both always relevant, no branch needed : the radio
 * routes facilities to the right handling process (paper vs. DEEE vs. furniture) while the
 * localisation field stays meaningful regardless of which is picked. Same "all fields answered every
 * time" shape as the pilot (`AbroadMissionFormBuilder`, issue #208), computed title built the same
 * way (`AnswerTagProvider::getTagForQuestion()` / `FormTagProvider::getTagForForm()`, never
 * hand-written `<span>` markup). Also adds the "Précisions complémentaires" free-text field every
 * class in this generalization wave adds, replacing the generic Description these smart forms no
 * longer have.
 */
class RecyclingRequestFormBuilder
{
    private const FORM_NAME = 'Demande liée au tri ou au recyclage';

    private const BRANCH_KEY = 'services_generaux';

    private const CATEGORY_PATH = ['RSE & Recyclage'];

    // Same icon `ServiceCatalogBuilder::BRANCH_ILLUSTRATIONS['services_generaux']` already gave
    // this branch's other forms.
    private const ILLUSTRATION = 'inventory';

    private const TYPE_DECHET_OPTIONS = [
        '1' => 'Papier / carton',
        '2' => 'Déchets électroniques (DEEE)',
        '3' => 'Mobilier',
        '4' => 'Autre',
    ];
    ...
    /**
     * @return array{typeDechet: Question, localisation: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $localisation->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Localisation à collecter', 'configurationglpiauto'),
        ...
        if (!$typeDechet->getID() || !$localisation->getID() || !$precisions->getID()) {
            return null;
        }

        return ['typeDechet' => $typeDechet, 'localisation' => $localisation];
    }

    /**
     * @param array{typeDechet: Question, localisation: Question} $questions
     */
    private function configureDestination(Form $form, int $itilCategoryId, array $questions): void
    {
        ...
        // "<Nom du formulaire> - <typeDechet>"
        $titleValue = $formProvider->getTagForForm($form)->html
            . ' - ' . $answerProvider->getTagForQuestion($questions['typeDechet'])->html;
