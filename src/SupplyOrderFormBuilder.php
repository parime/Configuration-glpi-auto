 ...
 * request) : "Demande d'achat ou commande de fournitures" (Achats & Logistique / Sourcing &
 * Commande), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES` (now flagged `'smart' => true` there and skipped by that
 * builder's own loop).
 *
 * No conditional branch : an order is always "what" and "how many", so both fields are always
 * relevant, same "all fields answered every time" shape as `AccessBadgeFormBuilder`. Also carries the
 * "Précisions complémentaires" free-text field every class in this generalization batch adds last,
 * replacing the generic Description field these smart forms no longer have.
 */
class SupplyOrderFormBuilder
{
    private const FORM_NAME = "Demande d'achat ou commande de fournitures";

    private const BRANCH_KEY = 'achats';

    private const CATEGORY_PATH = ['Sourcing & Commande'];
    ...
    /**
     * @return array{article: Question, quantite: Question, precisions: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $article->add([
            ...
            'name' => __('Article ou référence souhaité', 'configurationglpiauto'),
        ...
        $quantite->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Quantité', 'configurationglpiauto'),
        ...
        if (!$article->getID() || !$quantite->getID() || !$precisions->getID()) {
            return null;
        }

        return ['article' => $article, 'quantite' => $quantite, 'precisions' => $precisions];
    }

    /**
     * @param array{article: Question, quantite: Question, precisions: Question} $questions
     */
    private function configureDestination(Form $form, int $itilCategoryId, array $questions): void
    {
        ...
        // "<Nom du formulaire> - <article> (x<quantité>)"
