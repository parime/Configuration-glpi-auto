 ...
 * request) : "Demande de télétravail ou d'aménagement du temps de travail" (RH / Organisation du
 * travail), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES` (now flagged `'smart' => true` there and skipped by that
 * builder's own loop).
 *
 * The service's own name lists three genuinely different requests, each needing a different
 * follow-up : a one-off remote day needs a single date, a standing remote-work pattern needs a
 * recurring day of the week, and a schedule adjustment needs neither of those (it's handled through
 * the free-text "Précisions complémentaires" field instead, since a start-time change doesn't fit a
 * single structured field any better than free text would). Same `Question::visibility_strategy =
 * VISIBLE_IF` + `Glpi\Form\Condition\ConditionData` mechanism as `MeetingRoomFormBuilder` /
 * `VpnAccessFormBuilder`, one branch per option that needs its own field.
 *
 * Computed title deliberately only references the always-visible "type" answer, not the conditional
 * date/day fields : whichever branch wasn't chosen (including the "aménagement" branch, which has no
...
{
    private const FORM_NAME = "Demande de télétravail ou d'aménagement du temps de travail";
    ...
    private const PONCTUEL_OPTION_KEY = '1';

    private const REGULIER_OPTION_KEY = '2';
    ...
    private function addQuestions(Form $form): ?array
    {
        ...
        $ponctuelCondition = new ConditionData(
            item_uuid: $typeUuid,
            item_type: ConditionType::QUESTION->value,
            value_operator: ValueOperator::EQUALS->value,
            value: self::PONCTUEL_OPTION_KEY,
        );
        $regulierCondition = new ConditionData(
            item_uuid: $typeUuid,
            item_type: ConditionType::QUESTION->value,
            value_operator: ValueOperator::EQUALS->value,
            value: self::REGULIER_OPTION_KEY,
        );

        $datePonctuel = new Question();
        $datePonctuel->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Date concernée', 'configurationglpiauto'),
            ...
            'conditions' => json_encode([$ponctuelCondition->jsonSerialize()]),
        ]);

        $joursReguliers = new Question();
        $joursReguliers->add([
            'forms_sections_id' => $sectionId,
            'name' => __('Jour(s) souhaité(s)', 'configurationglpiauto'),
            'type' => QuestionTypeShortText::class,
            'is_mandatory' => 1,
            'vertical_rank' => 2,
            'visibility_strategy' => VisibilityStrategy::VISIBLE_IF->value,
            'conditions' => json_encode([$regulierCondition->jsonSerialize()]),
        ]);
        ...
        if (!$datePonctuel->getID() || !$joursReguliers->getID() || !$precisions->getID()) {
            return null;
        }
