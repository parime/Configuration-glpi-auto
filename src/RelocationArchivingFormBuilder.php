 ...
 * request) : "Demande de déménagement ou d'archivage" (Achats & Logistique / Déménagement &
 * Archivage), replacing the plain Title+Description entry that used to live in
 * `ServiceCatalogBuilder::SERVICES` (now flagged `'smart' => true` there and skipped by that
 * builder's own loop).
 *
 * No conditional branch : both a move and an archiving request need a location (departure/arrival
 * for a move, storage location for archiving), so the location field stays relevant regardless of
 * which type is picked — there's no follow-up field that only makes sense for one branch. Also
 ...
 */
class RelocationArchivingFormBuilder
{
    private const FORM_NAME = "Demande de déménagement ou d'archivage";

    private const BRANCH_KEY = 'achats';

    private const CATEGORY_PATH = ['Déménagement & Archivage'];
    ...
    private const TYPE_OPTIONS = [
        '1' => 'Déménagement de poste ou de bureau',
    ...
    /**
     * @return array{type: Question, localisation: Question, precisions: Question}|null
     */
    private function addQuestions(Form $form): ?array
    {
        ...
        $localisation->add([
            'forms_sections_id' => $sectionId,
            'name' => __("Localisation concernée (départ et/ou arrivée)", 'configurationglpiauto'),
        ...
        if (!$type->getID() || !$localisation->getID() || !$precisions->getID()) {
            return null;
        }

        return ['type' => $type, 'localisation' => $localisation, 'precisions' => $precisions];
    }

    /**
     * @param array{type: Question, localisation: Question, precisions: Question} $questions
     */
